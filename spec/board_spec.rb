...
describe Board do
    ...
    context "#is_entry_on_a_right_diagonal?" do 
    it "returns false if entry is not on a left diagonal" do
        expect(board.is_entry_on_a_right_diagonal?("A","2")).to eq(false)
        expect(board.is_entry_on_a_right_diagonal?("B","1")).to eq(false)
        expect(board.is_entry_on_a_right_diagonal?("C","3")).to eq(false)
    end
    it "returns true if entry is on a right diagonal" do
        expect(board.is_entry_on_a_right_diagonal?("A","3")).to eq(true)
        expect(board.is_entry_on_a_right_diagonal?("B","2")).to eq(true)
        expect(board.is_entry_on_a_right_diagonal?("C","1")).to eq(true)
    end
end
